...
jQuery.fn.extend({
  ...
  domManip: function(args, table, callback, options) {
    ...
    if (args[0] && args[0].nodeType) {
      ...
      while (
        i < argsLength &&
        !(tmplItem = jQuery.data(args[i++], 'tmplItem'))
      ) {}
      if (argsLength > 1) {
...
jQuery.extend({
  ...
  tmpl: function(tmpl, data, options, parentItem) {
    ...
    if (topLevel) {
      ...
      return jQuery(
        build(parentItem, null, parentItem.tmpl(jQuery, parentItem))
      );
    }
    if (!tmpl) {
      return []; // Could throw...
...
function build(tmplItem, nested, content) {
  ...
  ret.replace(/^\s*([^<\s][^<]*)?(<[\w\W]+>)([^>]*[^>\s])?\s*$/, function(
    all,
    before,
    middle,
    after
...
function tiHtml(filter, textOnly) {
  ...
  return jQuery.map(
    jQuery(jQuery.isArray(wrapped) ? wrapped.join('') : wrapped).filter(
      filter || '*'
    ),
    function(e) {
      return textOnly
