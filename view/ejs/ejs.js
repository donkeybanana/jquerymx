...
var scan = function(scanner, source, block) {
  ...
  compile = function(source, left, name) {
    ...
    scan(makeScanner(left, left === '[' ? ']' : '>'), source || '', function(
      token,
      scanner
    ) {
            ...
            switch (startTag) {
              ...
              case scanner.eLeft:
                ...
                if (quickFunc.test(content)) {
                  ...
                  content =
                    'function(__){var ' + parts[1] + '=$(__);' + parts[2] + '}';
                }
                buff.push(
                  insert_cmd,
                  'jQuery.EJS.clean(',
                  content,
                  bn ? startTxt : doubleParen
                );
                break;
              case scanner.eeLeft:
                // <%== content
...
//     return (
//       'jQuery.EJS(function(_CONTEXT,_VIEW) { ' +
//       new EJS({
//         text: src,
//         name: id
//       }).template.out +
...
$View.register({
  suffix: 'ejs',
  script: function(id, src) {
    return `function(){ console.error('This is a placeholder for view #${id}'); }`;
  },
  renderer: function(id, text) {
    const renderer = ejs.compile(text);
    return function(data, helpers) {
      const locals = { ...data, plugin: EJS.Helpers.prototype.plugin };
