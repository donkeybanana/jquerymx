...
var bind = function(el, ev, callback) {
  ...
  shifter = function shifter(context, name) {
    ...
    return function() {
      ...
      return method.apply(
        context,
        [this.nodeName ? $(this) : this].concat(slice.call(arguments, 0))
      );
    };
  },
  // matches dots
...
class $Controller {
  static fullName = '$Controller';
  static shortName = this.fullName;
  static _fullName = '$_controller';
  static _shortName = this._fullName;
  ...
  static isAction(name) {
    return actionMatcher.test(name);
  }
  ...
  constructor(el, opts) {
    this.element = $(el).addClass(this._fullName);
    (data(this.element) || data(this.element, {}))[this._fullName] = this;

    this.options = opts;
    this.options = extend(extend(true, {}, this.constructor.defaults), options);
  ...
  destroy() {
    if (this._destroyed) {
      throw this.shortName + ' controller already deleted';
    }

    this._destroyed = true;

    this.element.removeClass(this._fullName);

    this._unbind();

    delete this._actions;
    delete this.element.data('controllers')[this.fullName];
...
each(
  'change click contextmenu dblclick keydown keyup keypress mousedown mousemove mouseout mouseover mouseup reset resize scroll select submit focusin focusout mouseenter mouseleave'.split(
    ' '
  ),
  function(i, v) {
...
var i,
  isAControllerOf = function(instance, controllers) {
    for (i = 0; i < controllers.length; i++) {
      if (
        typeof controllers[i] == 'string'
          ? instance[STR_CONSTRUCTOR]._shortName == controllers[i]
