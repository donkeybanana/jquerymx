...
$.Class(
   ...
   */
  {
    ...
    destroy: function(success, error) {
      ...
      if (ids.length) {
        this.constructor.destroy(
          ids,
          () => {
            const destroyed = ids.map(id => {
              const item = this.get(id)[0];
              item.destroyed();
              return new item.constructor(item.attrs());
            });
            typeof success === 'function' && success(destroyed);
          },
          error
        );
      } else {
    ...
    bindings: function(items) {
      var self = this;
      $(items)
        .bind('destroyed' + this._namespace, function() {
          //remove from me
