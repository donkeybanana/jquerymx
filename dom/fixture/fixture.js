...
var typeTest = /^(script|json|test|jsonp)$/,
  ...
  getId = function(settings) {
    ...
    if (id === undefined) {
      // if still not set, guess a random number
...
$.extend($.fixture, {
  ...
  _getData: function(fixtureUrl, url) {
    var order = [],
      ...
      res = new RegExp(
        fixtureUrlAdjusted.replace(replacer, function(whole, part) {
          order.push(part);
          return '([^/]+)';
        }) + '$'
      ).exec(url),
  ...
  make: function(types, count, make, filter) {
    ...
    $.fixture['-' + types[0]] = function(settings) {
      ...
      for (var param in settings.data) {
        i = 0;
        if (
          settings.data[param] !== undefined && // don't do this if the value of the param is null (ignore it)
