...
var convertType = function(type) {
    return type
      .replace(/([a-z])([a-z]+)/gi, function(all, first, next) {
...
$.Range = function(range) {
  ...
  if (!range || range.nodeType) {
    ...
    range.originalEvent &&
    range.originalEvent.touches &&
    range.originalEvent.touches.length
  ) {
    this.moveToPoint(range.originalEvent.touches[0]);
  } else if (
    range.originalEvent &&
    range.originalEvent.changedTouches &&
    range.originalEvent.changedTouches.length
  ) {
...
$.extend(
  ...
  /** @prototype **/
  {
     ...
     */
    toString: function() {
      return typeof this.range.text == 'string'
        ? this.range.text
        : this.range.toString();
    },
    /**
        ...
        if (this.range.setStart) {
          if (typeof set == 'number') {
            ...
            this.range.setStart(
              ...
              this.range.startOffset + parseInt(set, 10)
            );
          } else {
     ...
     */
    rects: function(from) {
      var rects = $.makeArray(this.range.getClientRects()).sort(function(
          rect1,
          rect2
        ) {
...
(function() {
  ...
  fn.compare = range.compareBoundaryPoints
    ? function(type, range) {
        return this.range.compareBoundaryPoints(
          this.window().Range[reverse(type)],
          range.range
        );
      }
    : function(type, range) {
  ...
  fn.move = range.setStart
    ? function(type, range) {
        ...
        switch (type) {
          case 'START_TO_END':
            this.range.setStart(
              rangesRange.endContainer,
              rangesRange.endOffset
            );
            break;
          case 'START_TO_START':
            this.range.setStart(
              rangesRange.startContainer,
              rangesRange.startOffset
            );
            break;
          case 'END_TO_END':
  ...
  var cloneFunc = range.cloneRange ? 'cloneRange' : 'duplicate',
    selectFunc = range.selectNodeContents
      ? 'selectNodeContents'
      : 'moveToElementText';
   ...
   */
  fn.select = range.selectNodeContents
    ? function(el) {
        if (!el) {
          this.window()
            .getSelection()
...
var iterate = function(elems, cb) {
  ...
  isWhitespace = function(el) {
    ...
    return supportWhitespace
      ? el.isElementContentWhitespace
  ...
  within = function(rect, point) {
    return (
      ...
      rect.left + rect.width >= point.clientX &&
